...
An example usage of the car access command is
\[\textsf{client} --> \textsf{[0x04:1]} \;\;  \textsf{[0x00:1]} \]
\textsf{Command Code}, $\textsf{[0x04:1]}$, denotes an identifier/code for a command which is used to access to a car with identifier ,  \textsf{[0x00:1]}.
...
\hline
\textbf{Command Code} &\textbf{Device ID}  & \textbf{States Code} &  \textbf{Actions Code} \\
\hline
\hline

\textsf{[0x05:1]} -- Utility Request Command &     \textsf{[0x00:1]} -- Radio    &  \textsf{[0x00:1]} -- on   & \textsf{[0x00:1]} -- turn on  \\
...
\hline
\end{tabular}}
\caption{List of devices and their encodings.}
\end{table}
